package it.luca.disp.streaming.model;

public final class DatePattern {

    public static final String DEFAULT_DATE = "yyyy-MM-dd";
    public static final String DEFAULT_MONTH = "yyyy-MM";

    public static final String CONDUZIONE_DRE = "yyyy-MM-dd HH:mm:ss";
    public static final String CONDUZIONE_DRIF = "yyyy-MM-dd HH:mm:ss";

    public static final String INT002_GIORNO_ORA_RIFERIMENTO = "dd/MM/yyyy HH:mm";

    public static final String JARVIS_DATA_DI_CREAZIONE = "dd/MM/yyyy HH:mm";
    public static final String JARVIS_DATA_PROCEDURA = "dd/MM/yyyy";
    public static final String JARVIS_GIORNO_GAS = "dd/MM/yyyy";

    public static final String WEBDISP_DATA_ORA_INVIO = "dd/MM/yyyy HH:mm:ss";
    public static final String WEBDISP_DATA_ELABORAZIONE = "dd/MM/yyyy HH:mm:ss";
    public static final String WEBDISP_DATA_DECORRENZA = "dd/MM/yyyy HH:mm:ss";
}
